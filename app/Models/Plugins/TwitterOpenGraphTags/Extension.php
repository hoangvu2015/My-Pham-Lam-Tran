...
namespace Antoree\Models\Plugins\TwitterOpenGraphTags;
...
{
    const EXTENSION_NAME = 'twitter_open_graph_tags';
    const EXTENSION_DISPLAY_NAME = 'Twitter Open Graph Tags';
    const EXTENSION_DESCRIPTION = 'Set up Twitter Open Graph Tags';
    ...
    public function render()
    {
        ...
        return $this->parseCollection($this->convertToTwitter($data));
    }
    ...
    protected function parseItem($property, $item)
    {
        ...
        return PHP_EOL . t_metaName($property, $item);
    }
    ...
    {
        return PHP_EOL . t_metaName('twitter:image:src', $image);
    }
    ...
    protected function parseContents($property, array $contents)
    {
        ...
        foreach ($contents as $content) {
            $output .= PHP_EOL . t_metaName($property, $content);
        }
        return $output;
    }
     ...
     * @return mixed
     */
    protected function convertToTwitter($data)
    {
        //Because default content filter is only for Facebook Open Graph
        //Convert from FB to twitter
        ...
        $data['twitter:image:src'] = $data['og:image'];
        unset($data['og:image']);
