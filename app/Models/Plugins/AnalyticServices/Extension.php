...
namespace Antoree\Models\Plugins\AnalyticServices;
...
ga(\'send\', \'pageview\');';
    ...
    {
        return t_js('//www.google-analytics.com/analytics.js', true);
    }
...
mixpanel.init(\'' . $this->mixPanelToken . '\');';
    ...
    public function register()
    {
        if (!$this->cacheEnable) {
            ...
            enqueue_theme_footer(t_js(asset('public/js/analytic_services.js')), 'analytic_services');
    ...
    public function cacheScripts()
    {
        if ($this->cacheEnable) {
            ...
            if (!empty($cache)) {
                Storage::disk('public')->put('js/analytic_services.js', $cache);
            }
        }
    }
