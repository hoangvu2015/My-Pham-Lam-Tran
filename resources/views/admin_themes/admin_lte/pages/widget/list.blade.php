...
@section('page_breadcrumb')
    <ol class="breadcrumb">
        ...
        <li><a href="{{ localizedAdminURL('widgets') }}">{{ trans('pages.admin_widgets_title') }}</a></li>
    </ol>
@endsection
@section('lib_styles')
    ...
    <div class="modal fade" id="clone-modal" tabindex="-1" role="dialog" aria-labelledby="clone-modal-title">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="post" action="{{ localizedAdminURL('widgets/clone') }}?{{ $rdr_param }}">
                    {{ csrf_field() }}
                    ...
                    <div id="clone-modal-content" class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <select id="inputClonePlaceHolder" class="form-control select2" name="placeholder" style="width: 100%">
                    ...
                    <form method="post">
                        ...
                        <div class="row">
                            <div class="form-group col-xs-12 col-sm-4">
                                ...
                                <select id="inputWidget" class="form-control select2" name="widget" style="width: 100%">
                            ...
                            <div class="form-group col-xs-12 col-sm-4">
                                ...
                                <select id="inputPlaceHolder" class="form-control select2" name="placeholder" style="width: 100%">
                                    @foreach($placeholders as $name => $display_name)
                                        <option value="{{ $name }}">{{ $display_name }}</option>
                                    @endforeach
                                </select>
                ...
                <div class="box-body">
                    ...
                    @foreach($themeWidgets as $themeWidget)
                        <li data-widget="{{ $themeWidget->id }}">
                            ...
                            @endif
                            <div class="tools">
                                <a href="{{ localizedAdminURL('widgets/{id}/edit', ['id'=> $themeWidget->id]) }}">{{ trans('form.action_edit') }}</a> |
                                @if($themeWidget->active)
                                    <a href="{{ localizedAdminURL('widgets/{id}/deactivate', ['id'=> $themeWidget->id]) }}?{{ $rdr_param }}">{{ trans('form.action_deactivate') }}</a> |
                                @else
                                    <a href="{{ localizedAdminURL('widgets/{id}/activate', ['id'=> $themeWidget->id]) }}?{{ $rdr_param }}">{{ trans('form.action_activate') }}</a> |
                                @endif
                                <a class="clone" href="#">{{ trans('form.action_clone') }}</a> |
                                <a class="delete" href="{{ localizedAdminURL('widgets/{id}/delete', ['id'=> $themeWidget->id]) }}?{{ $rdr_param }}">{{ trans('form.action_delete') }}</a>
                            </div>
                        </li>
                    @endforeach
                    </ul>
