...
<div class="row">
    <form class="check-slug" method="post" action="{{ localizedAdminURL('link/categories/update')}}">
        {!! csrf_field() !!}
        <input type="hidden" name="id" value="{{ $current_link_category->id }}">
        <div class="col-xs-12">
            <div class="margin-bottom">
                ...
                <a class="btn btn-warning delete" href="{{ localizedAdminURL('link/categories/{id}/delete', ['id'=> $current_link_category->id]) }}">
                    {{ trans('form.action_delete') }}
                </a>
                <a class="btn btn-success" href="{{ localizedAdminURL('link/categories/{id}/sort', ['id' => $current_link_category->id]) }}">
                    {{ trans('form.action_sort') }}
                </a>
            </div>
            <h4 class="box-title">{{ trans('form.action_edit') }} {{ trans_choice('label.link_category_lc', 1) }}</h4>
        @if (count($errors) > 0)
        ...
        @endif
                ...
                <div class="col-xs-12 col-sm-6 col-md-4">
                    <div class="form-group">
                        ...
                        @foreach($categories as $category)
                            @if($category->id!=$current_link_category->id)
                                <option value="{{ $category->id }}"{{ $category->id==$current_link_category->parent_id ? ' selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endif
                        @endforeach
            ...
            <div class="nav-tabs-custom">
                ...
                <div class="tab-content">
                    @foreach(allSupportedLocales() as $locale => $properties)
                        <?php
                        $trans = $current_link_category->translate($locale);
            ...
            <div>
                ...
                <a role="button" class="btn btn-warning pull-right" href="{{ localizedAdminURL('link/categories') }}">{{ trans('form.action_cancel') }}</a>
            </div>
        </div>
    </form>
</div>
