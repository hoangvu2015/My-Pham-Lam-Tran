...
@section('site_description', $site_desc)
...
<form action="{{ localizedURL('auth/register') }}" method="post">
    ...
    @else
    <div class="form-group icheck hidden">
    @endif
        {{ trans('label.you_are') }}: &nbsp;&nbsp;
    ...
    @if(isset ($session_freetest))
    <div class="form-group">
        <input type="hidden" value="{{ $session_freetest }}" class="form-control" placeholder="session_freetest" name="session_freetest">
    </div>
    @endif
    @if (count($errors) > 0)
    @if(session('freeError'))
    <div class="form-group">
        <input type="hidden" value="{{ session('freeError') }}" class="form-control" placeholder="session_freetest" name="session_freetest">
    </div>
    @endif
    ...
    <div class="row">
        ...
        <div class="col-xs-4">
            ...
            @else
                @if($site_locale == 'vi')
                <button id="btn-register" onclick="ga('send', {
                  hitType: 'event',
                  eventCategory: 'BecomeATutor',
                  eventAction: 'ClickRegisterAccountButton',
                  eventLabel: 'ButtonVn'
              });" type="submit" class="btn btn-primary btn-block btn-flat">{{ trans('form.action_register') }}</button>
