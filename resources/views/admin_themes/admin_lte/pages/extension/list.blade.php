...
@section('page_breadcrumb')
    <ol class="breadcrumb">
        ...
        <li><a href="{{ localizedAdminURL('topics') }}">{{ trans('pages.admin_extensions_title') }}</a></li>
    </ol>
@endsection
                                    ...
                                    <td>
                                        @if($extension['activated'])
                                            @if($extension['editable'])
                                                <a href="{{ localizedAdminURL('extensions/{name}/edit', ['name' => $extension['name']]) }}">{{ trans('form.action_edit') }}</a>
                                            @endif
                                            @if(!$extension['static'])
                                                <a class="deactivate" href="{{ localizedAdminURL('extensions/{name}/deactivate', ['name'=> $extension['name']]) }}?{{ $rdr_param }}">{{ trans('form.action_deactivate') }}</a>
                                            @endif
                                        @else
                                            <a class="activate" href="{{ localizedAdminURL('extensions/{name}/activate', ['name' => $extension['name']]) }}?{{ $rdr_param }}">{{ trans('form.action_activate') }}</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
