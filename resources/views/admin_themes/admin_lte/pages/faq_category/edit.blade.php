...
@section('page_title', trans('pages.admin_faq_categories_title'))
@section('page_description', trans('pages.admin_faq_categories_desc'))
@section('page_breadcrumb')
<ol class="breadcrumb">
    <li><a href="{{ adminHomeURL() }}"><i class="fa fa-dashboard"></i> {{ trans('pages.page_home_title') }}</a></li>
    <li><a href="{{ localizedAdminURL('faq/categories') }}">{{ trans('pages.admin_faq_categories_title') }}</a></li>
</ol>
@endsection
@section('lib_styles')
...
<div class="row">
    <form class="check-slug" method="post" action="{{ localizedAdminURL('faq/categories/update')}}">
        {!! csrf_field() !!}
        <input type="hidden" name="id" value="{{ $current_category->id }}">
        <div class="col-xs-12">
            <div class="margin-bottom">
                <a class="btn btn-primary" href="{{ localizedAdminURL('faq/categories/add') }}">
                    {{ trans('form.action_add') }} {{ trans_choice('label.category_lc', 1) }}
                </a>
                <a class="btn btn-warning delete" href="{{ localizedAdminURL('faq/categories/{id}/delete', ['id'=> $current_category->id]) }}">
        ...
        @endif
            ...
            <div>
                ...
                <a role="button" class="btn btn-warning pull-right" href="{{ localizedAdminURL('faq/categories') }}">{{ trans('form.action_cancel') }}</a>
            </div>
        </div>
    </form>
</div>
